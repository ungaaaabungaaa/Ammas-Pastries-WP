...
function getShippingInfo (data, storeName, storeId, invoiceNumber, initDeliveryDate, totalAmount) {
  ...
  if (data.meta_data) {
    ...
    deliveryDate = `${date}T${time}:00.000`
  }
  ...
  const orderInfo = {
    ...
    "delivery": {
        ...
        "address": {
          ...
          "addressLine": shipping.address_1,
          "city": shipping.city,
    ...
    "payments": [{
      ...
      "amount": (data.status === 'pending' || data.status === 'processing') ? 0 : parseFloat(totalAmount),
...
function createOrderObject (data, storeName, storeId) {
  if (data) {
    ...
    const items = lineItems.map(item => {
      let itemData = {
        "shortName": item.parent_name,
        "longName": item.parent_name,
        "variants": item.variantId,
        "skuCode": item.product_id,
      ...
      // Theme cake item data update ------------------
      let itemName = ''
      metaData.forEach(({key, display_value}) => {
        if (key === 'pa_flavors') {
          itemName += display_value + ', '
        }
        if (key === 'pa_choose') {
          itemName += display_value
        }
    ...
    if (backOrders.length && normalOrders.length) {
      ...
      const normalOrderInfo = getShippingInfo(data, storeName, storeId, `${invoiceNumber}_1`, deliveryDate.toISOString(), normalOrdersTotal)
      ...
      const backOrderInfo = getShippingInfo(data, storeName, storeId, `${invoiceNumber}_2`, backOrderDeliveryDate.toISOString(), backOrdersTotal)
      ...
      const orderInfo = getShippingInfo(data, storeName, storeId, invoiceNumber, deliveryDate, data.total)
...
function createOrder (data, outlet) {
  return new Promise((resolve, reject) => {
    // Do not create order if status is processing or pending for india(INR)
    if (data.currency === 'INR' && (data.status === 'pending' || data.status === 'processing')) {
